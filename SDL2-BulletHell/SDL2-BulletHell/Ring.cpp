...
Ring::Ring(const Vector2d& location, int nb_bullets, double force_magnitude, double center_offset, double seed_angle, double angular_acceleration, SDL_Texture* shared_texture)
{
	...
	for (int i = 0; i < nb_bullets; ++i) {
		...
		Bullet* bullet = DBG_NEW Bullet(location_offset, angle_increment, angular_acceleration, force_magnitude, shared_texture);
		add(bullet);
